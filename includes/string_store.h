...
///Creates a new string out of a raw str/char*
string CreateString(str Data){
    string String;
    String.data = Data;
    String.length = string_length(Data);
...
string_store* string_store_create(stack_alloc* stack){
    string_store store = { stack, 0, NULL, NULL };
    return stack_push(stack, &store, sizeof(string_store));
}
...
string* string_store_get(string_store* Store, u32 index){
    string* String = stack_get(Store->stack, sizeof(string) + index);
    return String;
}
